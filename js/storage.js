// Robust Storage: IndexedDB → LocalStorage → in-memory
...
class IDBLayer {
  ...
  async init() {
    return new Promise((resolve, reject) => {
      ...
      const req = indexedDB.open('ble-scans-db', 1);
      req.onupgradeneeded = () => {
        const db = req.result;
...
class LSLayer {
  ...
  async add(rec){
    localStorage.setItem(this.prefix + rec.timestamp, JSON.stringify(rec));
  }
  async getAll(){
    const out = [];
