...
const state = {
  all: [],       // vollständige Liste aus Storage (plus Neu)
  filtered: [],  // aktuell gefilterte Ansicht
  heatOn: false
};
...
function validateRecord(rec) {
  // Enforce required schema, coerce types
  try {
    const out = {
...
function renderTable(records) {
  ...
  for (const r of records) {
    ...
    tr.appendChild(td(r.deviceName || ''));
...
function applyAndRenderFilters() {
  ...
  state.filtered = applyFilters(state.all, criteria);
...
function hookFilters() {
  ['#fltName','#fltRssiMin','#fltRssiMax','#fltFrom','#fltTo'].forEach(sel => {
    $(sel).addEventListener('input', applyAndRenderFilters);
    $(sel).addEventListener('change', applyAndRenderFilters);
  });
...
async function onStartScan() {
  ...
  scanner = new BLEScanner(setStatus, () => geo.getSnapshot(), async (recRaw) => {
    ...
    if (!rec) return;
    // persist
    // Edge case: identische timestamp-Kollision → minimalen Suffix
    if (state.all.some(r => r.timestamp === rec.timestamp)) {
      // Add +1 ms to ensure uniqueness
      rec.timestamp = new Date(Date.parse(rec.timestamp)+1).toISOString();
    }
    await storage.add(rec);
    state.all.push(rec);
    // live update
    state.filtered.push(rec);
    renderTable(state.filtered);
    mapView.addRecord(rec);
    if (state.heatOn) mapView.setHeat(state.filtered);
  });
